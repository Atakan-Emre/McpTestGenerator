...
class LintEngine:
    ...
    def lint(self, testcase: TestCase) -> LintResult:
        ...
        score, issues, suggestions = self._check_expected_result(testcase, score, issues, suggestions)
        score, issues, suggestions = self._check_test_data(testcase, score, issues, suggestions)
        score, issues, suggestions = self._check_classification(testcase, score, issues, suggestions)
    ...
    def _check_title(
        ...
        if any(word == title_lower.split()[0] if title_lower.split() else False for word in generic_words):
            issues.append(LintIssue(
        ...
        action_verbs = ["verify", "validate", "ensure", "confirm", "test", "check", "doğrula", "kontrol"]
    ...
    def _check_expected_result(
        ...
        measurable_indicators = [
            "should", "must", "will", "displays", "returns", "contains",
            "olmalı", "döner", "gösterir", "içerir", "eşittir"
        ]
        if not any(ind in exp.lower() for ind in measurable_indicators):
            suggestions.append("Beklenen sonuca ölçülebilir kriterler ekleyin (örn: 'Sayfa 3 saniyede yüklenmelidir')")
    ...
    def _check_test_data(
        ...
        needs_data = any(
            any(ind in step.action.lower() for ind in data_indicators)
            for step in tc.steps
        )
        ...
        if tc.test_data:
            ...
            if has_numeric and not has_boundary:
                suggestions.append("Numeric data için boundary değerler (min, max, sınır) eklemeyi düşünün")
