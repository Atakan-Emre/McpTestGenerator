...
def convert_to_xray(
    ...
    # Priority mapping
    if tc.priority:
        xray_payload["fields"]["priority"] = {
            "name": XRAY_PRIORITY_MAP.get(tc.priority, "Medium")
        }
    ...
    for field in unmapped:
        ...
        if value and (not isinstance(value, list) or value):
            ...
            warnings.append(f"'{field}' alanı Xray'e map edilemedi - custom field ekleyin veya description'a dahil edildi")
...
def _build_xray_description(tc: TestCase) -> str:
    ...
    parts.append("\n\n*Test Bilgileri:*")
    parts.append(f"• Senaryo Tipi: {tc.scenario_type.value if tc.scenario_type else 'Belirtilmemiş'}")
