...
def _select_testcases(
    ...
    for tc in sorted_cases:
        ...
        if current_duration + tc_duration > max_duration:
            include = False
            reason.append(f"Süre limiti aşılıyor ({current_duration + tc_duration} > {max_duration})")
...
def _build_coverage_summary(
    ...
    return {
        "modules": {
            ...
            "coverage_percent": round(len(covered_modules) / len(all_modules) * 100, 1) if all_modules else 0,
            "uncovered": list(all_modules - covered_modules),
        },
        "features": {
            "total": len(all_features),
            "covered": len(covered_features),
            "coverage_percent": round(len(covered_features) / len(all_features) * 100, 1) if all_features else 0,
...
def _generate_recommendations(
    ...
    critical_excluded = [
        tc for tc in excluded
        if tc.risk_level == RiskLevel.CRITICAL or tc.priority == Priority.P0
    ]
    ...
    selected_positive = sum(1 for tc in selected if tc.scenario_type and tc.scenario_type.value == "positive")
    selected_negative = sum(1 for tc in selected if tc.scenario_type and tc.scenario_type.value == "negative")
...
def coverage_report(
    ...
    if requirements:
        ...
        req_analysis = {
            ...
            "coverage_percent": round(len(covered) / len(requirements) * 100, 1) if requirements else 0,
    ...
    for tc in parsed_cases:
        if tc.scenario_type:
            scenario_distribution[tc.scenario_type.value] = scenario_distribution.get(tc.scenario_type.value, 0) + 1
    ...
    if total_tests < 10:
        recommendations.append("Test sayısı az. Daha kapsamlı test coverage için ek test case'ler oluşturun.")
    
    if scenario_distribution["boundary"] == 0:
        recommendations.append("Boundary test'ler yok. Sınır değer analizi ile test case'ler ekleyin.")
    
    if not any(tc.test_data for tc in parsed_cases):
        recommendations.append("Test data tanımlı değil. Data-driven testing yaklaşımını uygulayın.")
